...
class Database:
    ...
    def _ensure_message_schema(self) -> None:
        self.conn.executescript(
            ...
            CREATE INDEX IF NOT EXISTS idx_message_threads_updated ON message_threads(updated_at DESC, id DESC);
            ...
            CREATE INDEX IF NOT EXISTS idx_message_thread_members_user ON message_thread_members(user_id, thread_id);
            """
        )
        self._ensure_column("dm_messages", "thread_id", "INTEGER")
        self.conn.execute("CREATE INDEX IF NOT EXISTS idx_dm_thread ON dm_messages(thread_id, created_at, id)")
        self.conn.execute("UPDATE dm_messages SET body='' WHERE body IS NULL")
        self.conn.execute("UPDATE message_threads SET kind=COALESCE(NULLIF(kind,''), 'dm')")
        self.conn.execute("UPDATE message_threads SET name='' WHERE name IS NULL")
        self.conn.execute("UPDATE message_threads SET deleted=0 WHERE deleted IS NULL")
        self._backfill_message_threads_locked()

    def _dm_thread_key(self, user_a: int, user_b: int) -> str:
    ...
    def _message_thread_row_locked(self, thread_id: int) -> Optional[sqlite3.Row]:
        return self._one(
            """
            SELECT id, kind, name, thread_key, created_by, created_at, updated_at, deleted
    ...
    def _thread_member_ids_locked(self, thread_id: int) -> List[int]:
        rows = self._all(
            "SELECT user_id FROM message_thread_members WHERE thread_id=? ORDER BY user_id ASC",
            (int(thread_id),),
        )
        return [int(row["user_id"]) for row in rows]

    def _thread_members_locked(self, thread_id: int) -> List[Dict[str, Any]]:
        rows = self._all(
            """
            SELECT m.thread_id, m.user_id, m.joined_at, m.last_read_at, m.is_admin,
                   u.username, u.display_name, u.is_bot, u.is_admin AS is_site_admin
            FROM message_thread_members m
            JOIN users u ON u.id = m.user_id
            WHERE m.thread_id=?
            ORDER BY lower(u.display_name) ASC, lower(u.username) ASC, m.user_id ASC
            """,
            (int(thread_id),),
        )
            ...
            out.append(
                {
                    ...
                    "last_read_at": int(row["last_read_at"] or 0),
    ...
    def _ensure_thread_members_locked(self, thread_id: int, user_ids: Iterable[int], *, admin_user_id: Optional[int] = None) -> None:
        ts = now_ts()
        for user_id in sorted({int(user_id) for user_id in user_ids if int(user_id) > 0}):
            self.conn.execute(
                """
                INSERT OR IGNORE INTO message_thread_members (thread_id, user_id, joined_at, last_read_at, is_admin)
                VALUES (?, ?, ?, 0, ?)
                """,
                (int(thread_id), user_id, ts, 1 if admin_user_id and user_id == int(admin_user_id) else 0),
            )
        if admin_user_id:
            self.conn.execute(
                "UPDATE message_thread_members SET is_admin=1 WHERE thread_id=? AND user_id=?",
                (int(thread_id), int(admin_user_id)),
            )

    def _ensure_dm_thread_locked(self, user_a: int, user_b: int) -> sqlite3.Row:
        ids = sorted({int(user_a), int(user_b)})
        if not ids:
            raise ValueError("missing_dm_participants")
        if len(ids) == 1:
            ids = [ids[0], ids[0]]
        thread_key = self._dm_thread_key(ids[0], ids[1])
        row = self._one(
            """
            SELECT id, kind, name, thread_key, created_by, created_at, updated_at, deleted
            FROM message_threads
        ...
        if not row:
            ...
            cur = self.conn.execute(
                """
                INSERT INTO message_threads (kind, name, thread_key, created_by, created_at, updated_at, deleted)
                VALUES ('dm', '', ?, ?, ?, ?, 0)
                """,
                (thread_key, ids[0], ts, ts),
            )
            row = self._message_thread_row_locked(int(cur.lastrowid))
        if row:
            self.conn.execute("UPDATE message_threads SET deleted=0 WHERE id=?", (int(row["id"]),))
            self._ensure_thread_members_locked(int(row["id"]), ids, admin_user_id=ids[0])
        if not row:
            raise ValueError("dm_thread_create_failed")
        return row

    def _backfill_message_threads_locked(self) -> None:
        rows = self._all(
            """
            SELECT DISTINCT sender_id, recipient_id
            FROM dm_messages
            WHERE COALESCE(thread_id, 0)=0
            ORDER BY sender_id ASC, recipient_id ASC
            """
        )
        for row in rows:
            thread = self._ensure_dm_thread_locked(int(row["sender_id"]), int(row["recipient_id"]))
            self.conn.execute(
                """
                UPDATE dm_messages
                SET thread_id=?
                WHERE COALESCE(thread_id, 0)=0 AND sender_id=? AND recipient_id=?
                """,
                (int(thread["id"]), int(row["sender_id"]), int(row["recipient_id"])),
            )
        thread_rows = self._all("SELECT id, created_at FROM message_threads ORDER BY id ASC")
        for thread_row in thread_rows:
            last_row = self._one(
                "SELECT MAX(created_at) AS ts FROM dm_messages WHERE thread_id=? AND deleted=0",
                (int(thread_row["id"]),),
            )
            updated_at = int(last_row["ts"] if last_row and last_row["ts"] is not None else thread_row["created_at"])
            self.conn.execute(
                "UPDATE message_threads SET updated_at=? WHERE id=?",
                (updated_at, int(thread_row["id"])),
            )
        ...
        *,
        ...
        with self._lock, self.conn:
            ...
            if wallet_row:
                ...
                self._bootstrap_wallet_if_needed_locked(user_id, int(wallet_row["id"]))
    ...
    def can_access_file(self, user_id: int, file_id: int) -> bool:
        with self._lock:
            ...
            row = self._one(
                ...
                SELECT 1
                FROM dm_messages m
                JOIN message_thread_members mm ON mm.thread_id = m.thread_id
                WHERE m.file_id=? AND m.deleted=0 AND mm.user_id=?
                LIMIT 1
                """,
                (file_id, user_id),
            )
    ...
    def _user_in_thread_locked(self, user_id: int, thread_id: int) -> bool:
        row = self._one(
            "SELECT 1 FROM message_thread_members WHERE thread_id=? AND user_id=? LIMIT 1",
            (int(thread_id), int(user_id)),
        )
        return bool(row)

    def _message_payload_from_row_locked(self, row: sqlite3.Row) -> Dict[str, Any]:
        msg = dict(row)
        msg["id"] = int(msg["id"])
        msg["sender_id"] = int(msg["sender_id"])
        msg["recipient_id"] = int(msg["recipient_id"])
        msg["thread_id"] = int(msg.get("thread_id") or 0)
        msg["created_at"] = int(msg["created_at"])
        msg["deleted"] = bool(msg["deleted"])
        if msg.get("file_id"):
            file_row = self.get_file(int(msg["file_id"]))
            if file_row and int(file_row.get("deleted", 0)) == 0:
                msg["file"] = file_row
        return msg

    def _thread_last_message_locked(self, thread_id: int) -> Optional[Dict[str, Any]]:
        row = self._one(
            """
            SELECT m.id, m.sender_id, m.recipient_id, m.thread_id, m.body, m.file_id, m.created_at, m.deleted,
                   su.username AS sender_username, su.display_name AS sender_display_name,
                   t.kind AS thread_kind, t.name AS thread_name
            FROM dm_messages m
        ...
        return self._message_payload_from_row_locked(row) if row else None

    def _thread_payload_locked(self, thread_row: sqlite3.Row, viewer_user_id: int) -> Dict[str, Any]:
        thread_id = int(thread_row["id"])
        kind = str(thread_row["kind"] or "dm").strip().lower() or "dm"
        members = self._thread_members_locked(thread_id)
        last_message = self._thread_last_message_locked(thread_id)
        others = [member for member in members if int(member["id"]) != int(viewer_user_id)]
        direct_user = others[0] if others else (members[0] if members else None)
        title = (thread_row["name"] or "").strip()
        subtitle = ""
        if kind == "dm" and direct_user:
            title = direct_user["display_name"] or direct_user["username"]
            subtitle = f"@{direct_user['username']}"
        elif not title:
            names = [member["display_name"] or member["username"] for member in others[:3] or members[:3]]
            title = ", ".join(names) if names else "Untitled group"
        if kind == "group":
            subtitle = f"{len(members)} members"
        preview = "No messages yet."
        if last_message:
            preview = (last_message.get("body") or "").strip()
            if not preview and last_message.get("file"):
                preview = f"[attachment] {last_message['file']['original_name']}"
            if not preview:
                preview = "Attachment only"
        return {
            "id": thread_id,
            "kind": kind,
            "name": thread_row["name"] or "",
            "title": title,
            "subtitle": subtitle,
            "thread_key": thread_row["thread_key"],
            "created_at": int(thread_row["created_at"]),
            "updated_at": int(thread_row["updated_at"]),
            "created_by": self._basic_user_ref_locked(int(thread_row["created_by"] or 0) or None),
            ...
            "participants_preview": [
                ...
                for member in (others or members)[:4]
            ],
            "last_message": last_message,
            "last_message_id": int(last_message["id"]) if last_message else None,
            "last_sender": self._basic_user_ref_locked(int(last_message["sender_id"])) if last_message else None,
            "body": last_message.get("body", "") if last_message else "",
            "file_id": int(last_message["file_id"]) if last_message and last_message.get("file_id") else None,
            "preview": preview,
        }

    def message_thread_member_ids(self, thread_id: int) -> List[int]:
        with self._lock:
            return self._thread_member_ids_locked(thread_id)

    def list_message_threads(self, user_id: int, limit: int = 100) -> List[Dict[str, Any]]:
        with self._lock:
            rows = self._all(
                """
                SELECT t.id, t.kind, t.name, t.thread_key, t.created_by, t.created_at, t.updated_at, t.deleted
                FROM message_threads t
                JOIN message_thread_members m ON m.thread_id = t.id
                WHERE m.user_id=? AND COALESCE(t.deleted, 0)=0
                ORDER BY t.updated_at DESC, t.id DESC
                LIMIT ?
                """,
                (int(user_id), int(limit)),
            )
            return [self._thread_payload_locked(row, int(user_id)) for row in rows]

    def get_message_thread(self, user_id: int, thread_id: int) -> Optional[Dict[str, Any]]:
        with self._lock:
            if not self._user_in_thread_locked(int(user_id), int(thread_id)):
                return None
            row = self._message_thread_row_locked(int(thread_id))
            if not row or int(row["deleted"] or 0) == 1:
                return None
            return self._thread_payload_locked(row, int(user_id))

    def open_dm_thread(self, user_id: int, other_id: int) -> Optional[Dict[str, Any]]:
        if int(other_id) <= 0 or int(other_id) == int(user_id):
            return None
        with self._lock, self.conn:
            if not self._basic_user_ref_locked(int(other_id)):
                return None
            thread_row = self._ensure_dm_thread_locked(int(user_id), int(other_id))
            return self._thread_payload_locked(thread_row, int(user_id))

    def _list_thread_messages_locked(self, thread_id: int, limit: int = 300) -> List[Dict[str, Any]]:
        rows = self._all(
            """
            SELECT m.id, m.sender_id, m.recipient_id, m.thread_id, m.body, m.file_id, m.created_at, m.deleted,
                   su.username AS sender_username, su.display_name AS sender_display_name,
        ...
        return [self._message_payload_from_row_locked(row) for row in rows]

    def list_thread_messages(self, user_id: int, thread_id: int, limit: int = 300) -> List[Dict[str, Any]]:
        with self._lock:
            if not self._user_in_thread_locked(int(user_id), int(thread_id)):
                return []
            return self._list_thread_messages_locked(int(thread_id), limit=limit)
    ...
    def create_thread_message(
        ...
        ts = now_ts()
        with self._lock, self.conn:
            if not self._user_in_thread_locked(int(sender_id), int(thread_id)):
                return None
            thread_row = self._message_thread_row_locked(int(thread_id))
            if not thread_row or int(thread_row["deleted"] or 0) == 1:
                return None
            member_ids = self._thread_member_ids_locked(int(thread_id))
            if not member_ids:
                return None
            recipient_id = int(sender_id)
            if str(thread_row["kind"] or "dm") == "dm":
                others = [member_id for member_id in member_ids if int(member_id) != int(sender_id)]
                recipient_id = others[0] if others else int(sender_id)
            cur = self.conn.execute(
                """
                INSERT INTO dm_messages (sender_id, recipient_id, thread_id, body, file_id, created_at)
                VALUES (?, ?, ?, ?, ?, ?)
                """,
                (int(sender_id), recipient_id, int(thread_id), body, file_id, ts),
            )
            ...
            self.conn.execute(
                "UPDATE message_thread_members SET last_read_at=? WHERE thread_id=? AND user_id=?",
                (ts, int(thread_id), int(sender_id)),
            )
        return self.get_dm_message(msg_id)

    def create_group_thread(self, owner_id: int, name: str, member_ids: Iterable[int]) -> Dict[str, Any]:
        clean_name = (name or "").strip()[:80]
        unique_member_ids = sorted({int(member_id) for member_id in member_ids if int(member_id) > 0 and int(member_id) != int(owner_id)})
        if not clean_name:
            return {"error": "group_name_required"}
        if len(unique_member_ids) < 2:
            return {"error": "group_needs_two_members"}
        with self._lock, self.conn:
            ...
            rows = self._all(
                ...
                SELECT id
                FROM users
                WHERE id IN ({placeholders}) AND COALESCE(is_bot, 0)=0
                """,
                tuple(unique_member_ids),
            )
            found_ids = {int(row["id"]) for row in rows}
            if found_ids != set(unique_member_ids):
                return {"error": "group_member_not_found"}
            ts = now_ts()
            cur = self.conn.execute(
                """
                INSERT INTO message_threads (kind, name, thread_key, created_by, created_at, updated_at, deleted)
                VALUES ('group', ?, NULL, ?, ?, ?, 0)
                """,
                (clean_name, int(owner_id), ts, ts),
            )
            thread_id = int(cur.lastrowid)
            self._ensure_thread_members_locked(thread_id, [int(owner_id), *unique_member_ids], admin_user_id=int(owner_id))
            row = self._message_thread_row_locked(thread_id)
            if not row:
                return {"error": "group_create_failed"}
            return self._thread_payload_locked(row, int(owner_id))
    ...
    def create_dm_message(
        ...
        with self._lock, self.conn:
            thread_row = self._ensure_dm_thread_locked(int(sender_id), int(recipient_id))
        msg = self.create_thread_message(int(sender_id), int(thread_row["id"]), body, file_id=file_id)
        if not msg:
            raise ValueError("dm_message_create_failed")
        return msg
    ...
    def get_dm_message(self, msg_id: int) -> Optional[Dict[str, Any]]:
        with self._lock:
            ...
            if not row:
                return None
            return self._message_payload_from_row_locked(row)

    def list_dm_threads(self, user_id: int, limit: int = 100) -> List[Dict[str, Any]]:
        return self.list_message_threads(user_id, limit=limit)

    def list_dm_messages(self, user_a: int, user_b: int, limit: int = 200) -> List[Dict[str, Any]]:
        thread = self.open_dm_thread(int(user_a), int(user_b))
        if not thread:
            return []
        return self.list_thread_messages(int(user_a), int(thread["id"]), limit=limit)
    ...
    def _ensure_main_wallet_locked(self, user_id: int) -> int:
        ...
        if row:
            ...
            self._bootstrap_wallet_if_needed_locked(user_id, wallet_id)
            return wallet_id
        ts = now_ts()
        addr = self._new_wallet_address()
        wallet_id = self._next_wallet_id_locked()
        ...
        self._bootstrap_wallet_if_needed_locked(user_id, wid)
        return wid

    def _bootstrap_wallet_if_needed_locked(self, user_id: int, wallet_id: int) -> float:
        cc_id = self.cc_token_id()
        current_cc = self._wallet_amount_locked(wallet_id, cc_id)
        if current_cc > 0.000001:
            return current_cc
        has_token_position = self._one(
            "SELECT 1 FROM balances WHERE wallet_id=? AND token_id!=? AND amount > 0.0000001 LIMIT 1",
            (wallet_id, cc_id),
        )
        has_activity = self._one("SELECT 1 FROM transactions WHERE wallet_id=? LIMIT 1", (wallet_id,))
        if has_token_position or has_activity:
            return current_cc
        self._credit_balance_locked(wallet_id, cc_id, STARTER_WALLET_CC)
        self._insert_transaction_locked(
            user_id,
            wallet_id,
            "wallet_bootstrap",
            0,
            float(STARTER_WALLET_CC),
            {
                "reason": "starter_balance",
                "event_label": "Starter balance",
                "amount_cc": round(float(STARTER_WALLET_CC), 6),
            },
        )
        return self._wallet_amount_locked(wallet_id, cc_id)
    ...
    def _liquidity_position_value_cc_locked(self, token_id: int, share_units: float) -> float:
        share = float(share_units or 0.0)
        if share <= 0.0:
            return 0.0
        token_row = self._resolve_token_locked(token_id)
        if not token_row or str(token_row["symbol"] or "") == "CC":
            return 0.0
        total_units = self._liquidity_units_total_locked(token_id)
        if total_units <= 0.000001:
            return 0.0
        reserves = self._token_reserves_locked(token_row)
        pool_value_cc = max(0.0, reserves["cc"] * 2.0)
        return pool_value_cc * (share / total_units)
    ...
    def _wallet_total_cc_locked(self, wallet_id: int) -> float:
        ...
        for row in self._all("SELECT token_id, share_units FROM liquidity_positions WHERE wallet_id=?", (wallet_id,)):
            total += self._liquidity_position_value_cc_locked(int(row["token_id"]), float(row["share_units"] or 0.0))
        return total
    ...
    def _wallet_payload_locked(self, wallet_row: sqlite3.Row) -> Dict[str, Any]:
        ...
        for row in lp_rows:
            token_id = int(row["token_id"])
            share_units = float(row["share_units"] or 0.0)
            total_units = max(self._liquidity_units_total_locked(token_id), 0.000001)
            liquidity_positions.append(
                {
                    ...
                    "symbol": row["symbol"],
                    "slug": row["slug"],
                    "share_units": round(share_units, 6),
                    "share_pct": round(100.0 * share_units / total_units, 4),
                    "pool_value_cc": round(self._liquidity_position_value_cc_locked(token_id, share_units), 6),
                }
            )
        return {
            "id": wallet_id,
    ...
    def _insert_transaction_locked(
        self,
        user_id: int,
    ...
    def _wallet_list_item_locked(self, wallet_row: sqlite3.Row) -> Dict[str, Any]:
        ...
        return {
            ...
            "activity_count": int(activity_count_row["c"] if activity_count_row else 0),
            "total_value_cc": round(self._wallet_total_cc_locked(wallet_id), 6),
    ...
    def _explorer_transaction_summary_locked(self, item: Dict[str, Any]) -> str:
        ...
        if item.get("tx_kind") == "token_create":
            seed_liquidity_cc = float(meta.get("seed_liquidity_cc") or 0.0)
            creator_tokens = float(meta.get("creator_allocation_tokens") or item.get("amount") or 0.0)
            if seed_liquidity_cc > 0 and creator_tokens > 0:
                return f"Launch {symbol} | {seed_liquidity_cc:.2f} CC seeded, {creator_tokens:.2f} {symbol} minted"
            if seed_liquidity_cc > 0:
                return f"Launch {symbol} | {seed_liquidity_cc:.2f} CC seeded"
            return f"Launch {symbol}"
    ...
    def _execute_trade_locked(
        ...
        *,
        ...
        spot_price = max(0.0001, self._record_pool_price_locked(int(token_id), ts=ts))
        ...
        self._insert_transaction_locked(user_id, wallet_id, "trade", 0, float(delta_cc), tx_meta, created_at=ts)
    ...
    def manage_liquidity(
        ...
        *,
        ...
        with self._lock, self.conn:
            ...
            self._insert_transaction_locked(user_id, wallet_id, tx_kind, 0, float(delta_cc), meta, created_at=ts)
            explorer_tx_id = self._insert_explorer_transaction_locked(
                tx_kind=tx_kind,
                user_id=user_id,
                bot_account_id=actor_bot_id,
    ...
    def execute_trade(self, user_id: int, wallet_id: int, token_id: int, side: str, amount: float) -> Optional[Dict[str, Any]]:
        self.advance_market()
        with self._lock, self.conn:
            error_code = self._trade_error_code_locked(user_id, wallet_id, token_id, side, amount)
            if error_code:
                return {"error": error_code}
            result = self._execute_trade_locked(user_id, wallet_id, token_id, side, amount, reason="user_trade")
            if not result:
                return {"error": "trade_failed"}
            return result

    def _trade_error_code_locked(self, user_id: int, wallet_id: int, token_id: int, side: str, amount: float) -> Optional[str]:
        side_key = (side or "").strip().lower()
        trade_amount = float(amount or 0.0)
        if side_key not in {"buy", "sell"} or trade_amount <= 0:
            return "bad_trade"
        wallet = self._wallet_by_id_locked(int(wallet_id))
        if not wallet or bool(wallet["deleted"]) or int(wallet["user_id"] or 0) != int(user_id):
            return "wallet_not_found"
        token = self._resolve_token_locked(int(token_id))
        if not token:
            return "token_not_found"
        cc_id = self.cc_token_id()
        if int(token_id) == cc_id:
            return "cannot_trade_cc"
        params = self._parse_params(token["params_json"])
        fee_rate = clamp(float(params.get("trade_fee", TRADE_FEE_RATE)), 0.0025, 0.06)
        if side_key == "buy":
            quote = self._quote_buy_cost_locked(token, trade_amount, fee_rate)
            if not quote:
                return "trade_size_too_large"
            wallet_cc = self._wallet_amount_locked(int(wallet_id), cc_id)
            if wallet_cc + 1e-9 < float(quote["cc_total"]):
                return "insufficient_cc_balance"
            return None
        token_balance = self._wallet_amount_locked(int(wallet_id), int(token_id))
        if token_balance + 1e-9 < trade_amount:
            return "insufficient_token_balance"
        quote = self._quote_sell_proceeds_locked(token, trade_amount, fee_rate)
        if not quote:
            return "trade_size_too_large"
        return None
        ...
        with self._lock, self.conn:
            ...
            explorer_tx_id = self._insert_explorer_transaction_locked(
                ...
                meta={
                    ...
                    "seed_liquidity_cc": round(seed_liquidity_cc, 6),
                    "creator_allocation_tokens": round(creator_tokens, 6),
                    "creator_allocation_value_cc": round(creator_tokens * derived_launch_price, 6),
                },
                source_table="tokens",
                source_id=token_id,
                created_at=ts,
    ...
    def explorer_overview(self, limit_blocks: int = 10, limit_transactions: int = 15) -> Dict[str, Any]:
        with self._lock:
            mood_row = self._one("SELECT value FROM market_meta WHERE key='market_mood_regime'")
            pending_row = self._one("SELECT COUNT(*) AS c FROM explorer_transactions WHERE status='pending'")
            confirmed_row = self._one("SELECT COUNT(*) AS c FROM explorer_transactions WHERE status='confirmed'")
            pending_volume_row = self._one("SELECT COALESCE(SUM(ABS(value_cc)), 0) AS v FROM explorer_transactions WHERE status='pending'")
            latest_block_row = self._one("SELECT height, created_at FROM explorer_blocks ORDER BY height DESC LIMIT 1")
            counts = {
                ...
                "pending_transactions": int(pending_row["c"] if pending_row else 0),
                "confirmed_transactions": int(confirmed_row["c"] if confirmed_row else 0),
                ...
                "pending_volume_cc": round(float(pending_volume_row["v"] if pending_volume_row else 0.0), 6),
                "latest_block_height": int(latest_block_row["height"] if latest_block_row else 0),
                "latest_block_at": int(latest_block_row["created_at"] if latest_block_row else 0),
                "market_mood": {
                    "value": round(self._market_meta_float_locked("market_mood", 0.08), 6),
                    "regime": str(mood_row["value"] if mood_row else "balanced"),
                },
